...
namespace BA.Modules {
    internal static class Language
    {
        ...
        private static void Language_collectLanguageRootFolders(List<string> obj) {
            var path = Path.Combine(Path.GetDirectoryName(BAPlugin.instance.Info.Location), "Language");
        ...
        public static void PrintOutput(string fileName = "") {
            ...
            if (!string.IsNullOrEmpty(fileName)) {
                var path = Path.Combine(Directory.GetParent(BAPlugin.instance.Info.Location).FullName, "Language", "en", fileName);
